...
import { convertRenderedAssetsToComponentsInBatches, convertRenderedAssetsToIndexFile } from './files';
  ...
  async (
    ...
    if (exportConfiguration.ignoredAssetPaths.length > 0) {
      renderedAssets = renderedAssets.filter(
        (a) =>
          !isPathFilteredOut(exportConfiguration.ignoredAssetPaths, [
            ...a.group.path,
            a.group.name,
          ])
      );
    ...
    const components = await convertRenderedAssetsToComponentsInBatches(
      renderedAssets,
      sdk,
      exportConfiguration
    );
